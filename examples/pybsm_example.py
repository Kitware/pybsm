...
img_fname = './examples/data/M-41 Walker Bulldog (USA) width 319cm height 272cm.tiff'
...
ref_img.refl_values = np.array([.05,.5])
...
for ii in np.arange(4,ground_range.shape[0]):
    ...
    plt.title('groundRange: ' + str(ground_range[ii]) + ' km', fontdict={'fontsize': 6})
    scenario.ground_range = ground_range[ii]
    img_out = simulation.simulate_image(ref_img, sensor, scenario)[2]
    img_out = simulation.stretch_contrast_convert_8bit(img_out)
    plt.imshow(img_out,cmap='gray')
plt.savefig('./examples/refactor_branch/fig_ihaze-' + str(ihaze) + '_alt-' + str(altitude) + '.png')
