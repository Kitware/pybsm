# Contributing to `pybsm`

## Making a Contribution
Here we describe at a high level how to contribute to `pybsm`.
See the [`pybsm` README](README.md) file for additional information.

1.  Navigate to the official `pybsm` repository maintained [on JATIC GitLab](
    https://gitlab.jatic.net/jatic/kitware/pybsm).

2.  Fork `pybsm` into your GitLab user namespace and clone that onto
    your system.
...
5. On GitLab, visit your forked project page then go the "Merge Requests" tab
   ...
   dropdown, select the relevant branch from the `pybsm` repository. Next,
   click "Compare branches and continue". Finally, click the "Create merge
   request" button to create the request.

`pybsm` uses GitLab for code review and GitLab CI/CD for continuous
testing.
...
* Notebooks should also be runnable when executed in CI.
  ...
  in the appropriate dependencies (including `pybsm` itself) into the
