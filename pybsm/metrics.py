...
import pybsm.geospatial as geospatial
...
def niirs(sensor,scenario):
    ...
    nm.slantRange = geospatial.curvedEarthSlantRange(0.0,scenario.altitude,scenario.groundRange)

    ##########CONTRAST SNR CALCULATION#########
    #load the atmosphere model
    nm.atm = radiance.loadDatabaseAtmosphere(scenario.altitude,scenario.groundRange,scenario.ihaze)
    ...
    nm.snr = radiance.photonDetectorSNR(sensor, nm.radianceWavelengths,nm.tgtRadiance,nm.bkgRadiance)
    ...
    tgtNoise= np.sqrt(np.trapz(radiance.photonDetectionRate(nm.snr.tgtFPAirradiance-nm.snr.otherIrradiance,nm.sensor.wx, \
    nm.sensor.wy,nm.radianceWavelengths,nm.snr.qe),nm.radianceWavelengths)*nm.snr.intTime*sensor.ntdi)
    bkgNoise= np.sqrt(np.trapz(radiance.photonDetectionRate(nm.snr.bkgFPAirradiance-nm.snr.otherIrradiance,nm.sensor.wx, \
    ...
    scatrate,_,_ = radiance.signalRate(nm.radianceWavelengths,nm.atm[:,2]+nm.atm[:,4],nm.snr.optTrans, \
    ...
    nm.otf = otf.commonOTFs(sensor,scenario,nm.uu,nm.vv,nm.mtfwavelengths,nm.mtfweights,nm.slantRange,nm.snr.intTime)
    ...
    nm.elevAngle = np.pi/2-geospatial.nadirAngle(0.0,scenario.altitude,nm.slantRange)
...
def plotCommonMTFs(metrics,orientationAngle=0):
    ...
    sf = otf.sliceotf(.001*(1.0/metrics.sensor.f)*radfreq,orientationAngle)
