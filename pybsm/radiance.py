...
# pybsm imports
import pybsm.metrics as metrics
...
def photonDetectorSNR(sensor,radianceWavelengths, targetRadiance,
                      backgroundRadiance):
    ...
    #quantization noise
    snr.quantizationNoise = noise.quantizationNoise(sensor.maxN,sensor.bitdepth)
...
def reflectance2photoelectrons(atm, sensor, intTime, target_temp=300):
    ...
    for idx in np.arange(ref.size):
        ...
        backgroundRadiance = 0

        wavelengths = atm[:,0]

        optTrans = sensor.coldfilterTransmission*(1.0-sensor.eta**2)*resampleByWavelength(sensor.optTransWavelengths,sensor.opticsTransmission,wavelengths)

        qe = resampleByWavelength(sensor.qewavelengths, sensor.qe,
                                  wavelengths)
        ...
        otherIrradiance =  coldshieldSelfEmission(wavelengths,
                                                  sensor.coldshieldTemperature,
                                                  sensor.D,sensor.f)
        otherIrradiance = otherIrradiance + opticsSelfEmission(wavelengths,
                                                               sensor.opticsTemperature,
                                                               sensor.opticsEmissivity,
                                                               sensor.coldfilterTransmission,
                                                               sensor.D,
                                                               sensor.f)
        otherIrradiance = otherIrradiance + coldstopSelfEmission(wavelengths,
                                                                 sensor.coldfilterTemperature,
                                                                 sensor.coldfilterEmissivity,
                                                                 sensor.D,
                                                                 sensor.f)

        tgtNrate, tgtFPAirradiance, weights = signalRate(wavelengths,
                                                         targetRadiance,
                                                         optTrans,
                                                         sensor.D,
                                                         sensor.f,
    ...
    if sat > 1:
        print(f'Reducing integration time from {intTime} to {intTime/sat}'
...
def signalRate(wavelengths, targetRadiance, opticalTransmission, D, f, wx, wy,
               qe, otherIrradiance, darkCurrent):
