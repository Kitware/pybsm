...
class TestReflectance2Photoelectrons:
    ...
    def test_photon_detection_rate_empty_array(
        self, E: np.ndarray, wx: float, wy: float, wavelengths: np.ndarray, qe: np.ndarray
    ) -> None:
        """
    ...
    @pytest.mark.parametrize(
        "E, wx, wy, wavelengths, qe, expected",
        [
            (np.array([1.0]), 1.0, 1.0, np.array([1.0]), np.array([1.0]), np.array([5.03411665e24])),
            (
                np.array([1.0, 1.0]),
                1.0,
                1.0,
    ...
    def test_photon_detection_rate(
        self, E: np.ndarray, wx: float, wy: float, wavelengths: np.ndarray, qe: np.ndarray, expected: np.ndarray
    ) -> None:
        """
    ...
    def test_at_focal_plane_irradiance_zero_division(self, D: float, f: float, L: np.ndarray) -> None:
        """
    ...
    def test_at_focal_plane_irradiance(self, D: float, f: float, L: np.ndarray, expected: np.ndarray) -> None:
        """
        Cover cases with normal inputs and expected outputs
        """
        output = radiance.atFocalPlaneIrradiance(D, f, L)
    ...
    def test_blackbody_radiance_empty_array(self, lambda0: np.ndarray, T: float) -> None:
        """
        Cover cases where output is an empty array
        """
    ...
    def test_blackbody_radiance(self, lambda0: np.ndarray, T: float, expected: np.ndarray) -> None:
        """
        Cover cases with normal inputs and expected outputs
        """
        output = radiance.blackbodyRadiance(lambda0, T)
    ...
    @pytest.mark.parametrize(
        "wavelengths, values, newWavelengths",
        [
    ...
    def test_resample_by_wavelength_value_error(
        self, wavelengths: np.ndarray, values: np.ndarray, newWavelengths: np.ndarray
    ) -> None:
        """
    ...
    @pytest.mark.parametrize(
        "wavelengths, values, newWavelengths, expected",
        [
    ...
    @pytest.mark.parametrize(
        ("wavelengths, targetRadiance, opticalTransmission, D, f, wx, wy, qe, otherIrradiance," "darkCurrent"),
        [
    ...
    @pytest.mark.parametrize(
        (
            "wavelengths, targetRadiance, opticalTransmission, D, f, wx, wy, qe, otherIrradiance,"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                np.array([1.0, 1.0]),
                1.0,
                (1.0, np.array([1.62831853, 1.62831853]), np.array([8.19714543e24, 8.19714543e24])),
            ),
            (
                np.array([1.0, 1.0]),
                np.array([1.0, 1.0]),
                ...
                np.array([1.0, 1.0]),
                2.0,
                (2.0, np.array([1.62831853, 1.62831853]), np.array([8.19714543e24, 8.19714543e24])),
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "wavelengths, coldfilterTemperature, coldfilterEmissivity, D, f, expected",
        [
    ...
    @pytest.mark.parametrize(
        ("wavelengths, opticsTemperature, opticsEmissivity, coldfilterTransmission, D, f"),
        [
    ...
    def test_optics_self_emission_zero_division(
        ...
        """
        with pytest.raises(ZeroDivisionError):
            radiance.opticsSelfEmission(wavelengths, opticsTemperature, opticsEmissivity, coldfilterTransmission, D, f)
    ...
    def test_optics_self_emission_empty_array(
        ...
        Cover cases where output is an empty array
        """
        output = radiance.opticsSelfEmission(
            wavelengths, opticsTemperature, opticsEmissivity, coldfilterTransmission, D, f
        )
        assert output.size == 0

    @pytest.mark.parametrize(
        ("wavelengths, opticsTemperature, opticsEmissivity, coldfilterTransmission, D, f," "expected"),
        [
    ...
    @pytest.mark.parametrize(
        "wavelengths, coldshieldTemperature, D, f",
        [
    ...
    def test_coldshield_self_emission_zero_division(
        self, wavelengths: np.ndarray, coldshieldTemperature: float, D: float, f: float
    ) -> None:
        """
        Cover cases where ZeroDivisionError occurs
        """
    ...
    @pytest.mark.parametrize(
        "wavelengths, coldshieldTemperature, D, f, expected",
        [
    ...
    def test_coldshield_self_emission(
        self, wavelengths: np.ndarray, coldshieldTemperature: float, D: float, f: float, expected: np.ndarray
    ) -> None:
        """
        Cover cases with normal inputs and expected outputs
        """
    ...
    def test_total_radiance(
        self, atm: np.ndarray, reflectance: float, temperature: float, expected: np.ndarray
    ) -> None:
        """
        Cover cases with normal inputs and expected outputs
        """
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                utils.loadDatabaseAtmosphere(1000.0, 0.0, 1),
                0.0,
                0.0,
    ...
    @pytest.mark.parametrize(
        "atm, sensor, intTime, target_temp",
        [
    ...
    def test_reflectance_2_photoelectrons_index_error(
        self, atm: np.ndarray, sensor: Sensor, intTime: float, target_temp: int
    ) -> None:
        """
        Cover cases where IndexError occurs
        """
    ...
    @pytest.mark.parametrize(
        "atm, sensor, intTime, target_temp, expected",
        [
            (
                np.ones(shape=(6, 6)),
        ...
        [
            (
                ...
                (
                    ...
                    np.load("./tests/radiance/data/reflectance_2_photoelectrons_pe.npy"),
